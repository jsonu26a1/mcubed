...
impl SampleTree {
    ...
    // pub fn remove_experiment1(&mut self, key: u64) -> Option<u64> {
    //     todo!();
    //     // copy prelude from `remove()`
    ...
    //     /*
    //     the conditional logic is quite complicated; 
    //     */
    // }
    ...
    */
    pub fn remove(&mut self, key: u64) -> Option<u64> {
        ...
        */
        ...
        // TODO: I think we need a case where we merge current with just left or right (when at height 1)
        // if there's at least two leafs, we can always balance items between them so one leaf is never
        // below the minimum.
        // say order 10, min 5; left has 6, right has 5. if we remove from right, so it has 4, rather than
        // popping from left into right, we should just merge right into left.
        // I'm not sure if that's something we should handle as a new Case:: variant, or as a check within
        // [Left/Right]ToCurrent.
        // we might need to handle it within *ToCurrent; suppose current leaf is either first or last node;
        // if the only adjacent sibling has exactly minimum items, the *ToCurrent case would be invalid.
        // this might occur at any internal node, not just at height 1, where there's only two leaf nodes
        // on root. so this (merging only 2 nodes) is definitely something we need to handle.
        if let Some(ref left_sibling) = left_sibling && let Some(ref right_sibling) = right_sibling {
            ...
            if left_available + right_available < current_len {
                // we cannot split current's (leaf) items into left and right; so we must
                // steal items from left and right so current's len >= min_items
                // WAIT; current should require exactly one item to make it >= min,
                // so we should steal from which ever of left or right has more items.
                ...
                // NOTE: we don't need to remove anything from parent, but we do need to fix it's keys
            } else {
            ...
            // if left and right are None, parent is an internal node with only one leaf (current);
            // an internal node should never reach this state; even if parent is the root, it have
            // moved current into the root slot already.

            // (reword)
            // a leaf should always have at least one sibling (left or right). when an internal node
            // only has one child, it should merge with a sibling internal node, or if it is the root
            // node, replace itself in the root node slot with it's child
        ...
        match case {
            ...
            Case::MergeWithLeft => {
                ...
                parent.keys()[i - 1] = *left_sibling.keys().last().unwrap();
                // since right_sibling is None, leaf is the last edge in parent, and doesn't
                // have an associated key to remove.
                // parent.keys().remove(i);
                parent.edges().remove(i);
                // we don't return here because we need to check if parent needs re-balancing
            },
            Case::BalanceFromRight => {
                // remove at 0 from right, push to end of current
            ...
            Case::MergeWithRight => {
                ...
                parent.keys().remove(i);
                parent.edges().remove(i);
                // since left_sibling is None, leaf is the first edge in parent, and doesn't
                // have an associated key to update.
                // we don't return here because we need to check if parent needs re-balancing
            },
            Case::MergeThree => {
                // split current's items between left and right (and remove current from parent)
                let left_sibling = left_sibling.unwrap();
                let right_sibling = right_sibling.unwrap();
                let left_avail = max_items - left_sibling.keys().len();
                let right_avail = max_items - right_sibling.keys().len();
                // (left_avail + right_avail - leaf.keys().len()) / 2
                // ... if left_avail = 1, right_avail = 3, and leaf len = 4, the above equation would...
                // no, the case we want to handle would be left: 5, right: 5, leaf len: 3; we would want to
                // attempt to evenly split items between left and right... but also handle when left or right
                // is already full, if that makes sense.

                // oh, I think the Case::***ToCurrent should be modified, to handle cases with more than
                // one item; since that will be encountered when height = 1.

                let leaf_len = leaf.keys().len();
                let left_take = std::cmp::min(left_avail, leaf_len / 2);
                // let right_take = std::cmp::min(right_avail, leaf_len - left_take);
                left_sibling.keys().extend(leaf.keys().split_off(left_take).drain(..));
                left_sibling.values().extend(leaf.values().split_off(left_take).drain(..));
                leaf.keys().extend(right_sibling.keys().drain(..));
                leaf.values().extend(right_sibling.values().drain(..));
                std::mem::swap(&mut leaf.keys(), &mut right_sibling.keys());
                std::mem::swap(&mut leaf.values(), &mut right_sibling.values());
    ...
    pub fn remove_old1(&mut self, key: u64) -> Option<u64> {
        ...
        // if it has LESS than max items, right? we need to be checking BOTH.
        if let Some(ref right_sibling) = right_sibling
            && right_sibling.keys().len() < self.order as usize
            && right_sibling.keys().len() > min_items
        {
            // case 1)
            ...
            && left_sibling.keys().len() < self.order as usize
            && left_sibling.keys().len() > min_items
        {
            // case 2)
        ...
        */
        ...
        loop {
            ...
            if let Some(ref right_sibling) = right_sibling && right_sibling.keys().len() < self.order as usize {
                // case 1)
                let key = right_sibling.keys().remove(0);
            ...
            else if let Some(ref left_sibling) = left_sibling && left_sibling.keys().len() < self.order as usize {
                // case 2)
                let key = left_sibling.keys().pop().unwrap();
