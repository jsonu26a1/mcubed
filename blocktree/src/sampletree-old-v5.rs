...
impl SampleTree {
    ...
    pub fn iter_range(&self, range: impl RangeBounds<u64>) -> Vec<(u64, u64)> {
        ...
        loop {
            ...
            // let leaf_keys = leaf.keys();
            // if let Some(key) = end_key && *leaf_keys.last().unwrap() > key {
            //     let i = match leaf_keys.binary_search(&key) {
            //         Ok(i) => if let Bound::Excluded(_) = range.end_bound() && key == leaf_keys[i] {
            //                 if i == 0 {
            //                     break;
            //                 }
            ...
            //         Err(i) => i
            //     };
            //     out.extend(leaf_keys[..i].iter().map(|k| *k).zip(leaf.values()[..i].iter().map(|v| *v)));
            //     break;
            // } else {
            //     out.extend(leaf_keys[..].iter().map(|k| *k).zip(leaf.values()[..].iter().map(|v| *v)));
            //     next_leaf = leaf.next().clone();
        ...
        loop {
            ...
            match case {
                Case::BalanceFromLeft => {
                    ...
                    // let edge = left_sibling.edges().pop().unwrap();
                    // let mut last_edge = edge.clone();
                    // current.keys().insert(0, loop {
                    //     match last_edge {
                    //         EitherNode::Internal(internal) => {
                    //             last_edge = internal.edges().last().unwrap().clone();
                    //         },
                    //         EitherNode::Leaf(leaf) => break *leaf.keys().last().unwrap(),
                    //     }
                    // });
                ...
                Case::MergeWithLeft => {
                    ...
                    // let mut last_edge = left_sibling.edges().last().unwrap().clone();
                    // left_sibling.keys().push(loop {
                    //     match last_edge {
                    //         EitherNode::Internal(internal) => {
                    //             last_edge = internal.edges().last().unwrap().clone();
                    //         },
                ...
                Case::BalanceFromRight => {
                    ...
                    // let mut last_edge = current.edges().last().unwrap().clone();
                    // current.keys().push(loop {
                    //     match last_edge {
                    //         EitherNode::Internal(internal) => {
                    //             last_edge = internal.edges().last().unwrap().clone();
                    //         },
                ...
                Case::MergeThree => {
                    ...
                    /*
                    // let mut last_edge = rem_edges.edges().last().unwrap().clone();
                    // rem_keys.keys().push(loop {
                    //     match last_edge {
                    //         EitherNode::Internal(internal) => {
                    //             last_edge = internal.edges().last().unwrap().clone();
                    //         },
                    ...
                    // });
                    rem_keys.extend(right_sibling.keys().drain(..));
                    rem_edges.extend(right_sibling.edges().drain(..));
                    *right_sibling.keys() = rem_keys;
                    *right_sibling.edges() = rem_edges;
                    */
                    parent.keys()[i - 1] = *left_sibling.keys().last().unwrap();
                    parent.keys().remove(i);
                    parent.edges().remove(i);
...
impl EitherNode {
    ...
    //     match self {
    //         Self::Internal(n) => n.largest_key_in_subtree(),
    //         Self::Leaf(n) => *n.keys().last().unwrap(),
    //     }
    // }
}
